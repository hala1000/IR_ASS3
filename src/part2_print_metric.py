...
if __name__ == "__main__":
    ...
    query_file_path = './query_big/dev_queries.tsv'
    relevant_docs_file = './query_big/dev_query_results.csv'
    ...
    query_ids = queries_df['Query number'].tolist()[:1000]
    ...
    # Step 2: Calculate metrics for each k in [1, 3, 5, 10]
    for k in [1, 3, 5, 10]:
        logging.info(f"Calculating metrics for top-{k} retrieval")
        precisions = []
        recalls = []

        for query_id, retrieved_docs in retrieved_dic.items():
            # Use only the top-k retrieved documents
            top_k_retrieved_docs = retrieved_docs[:k]
            retrieved_relevant = len(set(top_k_retrieved_docs).intersection(set(relevant_docs.get(query_id, []))))

            precision = retrieved_relevant / k
            recall = retrieved_relevant / len(relevant_docs.get(query_id, [])) if query_id in relevant_docs else 0

            precisions.append(precision)
            recalls.append(recall)

        # Compute mean precision and recall for current k
        mean_precision = np.mean(precisions)
        mean_recall = np.mean(recalls)

        # Log results
        logging.info(f"Inverted Index - Mean Precision@{k}: {mean_precision:.4f}")
        logging.info(f"Inverted Index - Mean Recall@{k}: {mean_recall:.4f}")
